...
//   date: fall 2014
//   uses:
//        RtAudio by Gary Scavone (Audio Processing) ,
//        glm by Nate Robins [http://www.pobox.com/~ndr] (obj importing & models),
//        3DMushroom by alkhor [http://alkhor.deviantart.com/art/3D-mushroom-obj-376662783] (cool mushrooms )
//
//-----------------------------------------------------------------------------
#include "RtAudio.h"
#include "vector3d.h"
...
struct objModel
{
  ...
  objModel() {
    position.set(0,0,0);
    model = glmReadOBJ("obj_files/TheRock2.obj");
    rotation.set(10, 1, 0);
  ...
  void eyebrows(){
    ...
    glPushMatrix();
        ...
        // translate
        glTranslatef(  position.x, position.y + 2.2 , position.z );
  ...
  void ears()
  {
    ...
    vector<Vector3D > curr;
    curr.resize(g_windowSize/2);
    ...
    if(past.size() >= PAST_LENGTH)
      {
        past.pop_back();
      }
    past.push_front(curr);

    for(int i = 0; i < past.size(); i++){
      vector<Vector3D > c = past[i];
      // save transformation state
      glPushMatrix();
        // color
        glColor3f( .5, 1, .5 );
          ...
          glRotatef( rotation.y - i * .3, 0, 1, 0 );
          // start primitive
          glLineWidth( 4.0 );
          glBegin( GL_LINE_STRIP );
            for( int j = 0;j < c.size(); j ++)
              {
                Vector3D next = c[j];
                glVertex3f( next.x, next.y, position.z );
              }
...
int PERSISTANCE_MODE = 44032;
float BEAT_CONSTANT = 1.3;
deque<SAMPLE> sampleHistory;
bool beat;
bool isBeat( )
{
  SAMPLE instantEnergy = 0.0;
  //calculate average energy
  for( int i = 0; i < g_bufferSize; i++ )
  {
    instantEnergy = instantEnergy + pow (g_buffer[i], 2.0);
  }




  SAMPLE localAverageEnergy = 0.0;

  for (int i = 0; i < PERSISTANCE_MODE; i++)
  {

    localAverageEnergy = localAverageEnergy + pow (sampleHistory[i], 2.0);
  }

  localAverageEnergy = localAverageEnergy * (g_bufferSize / (SAMPLE)MY_SRATE);
  ...
  for(int i = 0 ;i < g_bufferSize; i ++)
  {
      ...
      sampleHistory.push_front(s);

  }
  sampleHistory.resize(PERSISTANCE_MODE);

  if(instantEnergy > BEAT_CONSTANT * localAverageEnergy){
    return TRUE;
  }else{
    return FALSE;
  }
...
void help()
{
    ...
    drawASCII();
    cerr << "----------------------------------------------------" << endl;
    cerr << "Josh Go Home (v1.0)" << endl;
    cerr << "Joshua J Coronado" << endl;
    cerr << "https://ccrma.stanford.edu/~jjcorona/classes/music256/hw2/" << endl;
    cerr << "----------------------------------------------------" << endl;
...
void displayFunc( )
{
    ...
    GLfloat g_light0_pos[4] = { XFun::rand2f(-6, 6), XFun::rand2f(-6, 6), XFun::rand2f(-6, 6), 1.0f };

    if(beat)glLightfv( GL_LIGHT0, GL_POSITION, g_light0_pos );
    l += .1;

    // clear the color and depth buffers
    glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );

    // go through my zthingz
